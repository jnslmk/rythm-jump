...
def test_game_beat_grid_refreshes_when_audio_duration_changes() -> None:
...
(() => {
  ...
  Object.defineProperty(audio, 'duration', {
    configurable: true,
    get() {
      return window.__audioDurationSeconds ?? 0;
    }
  });
...
""",
        ...
        result = page.evaluate(
          ...
          const before = beatGrid.style.gridTemplateColumns;

          window.__audioDurationSeconds = 2.4;
          document
            .getElementById('song-audio')
            .dispatchEvent(new Event('loadedmetadata'));
          await new Promise((resolve) => setTimeout(resolve, 50));

          return {
            before,
            after: beatGrid.style.gridTemplateColumns,
          };
        }""",
        )
        browser.close()
