...
test('left lane reaches the center hit zone at full progress', () => {
  assert.equal(projectBarHeadIndex(70, 1, 'left'), 34);
});

test('right lane reaches the center hit zone at full progress', () => {
  assert.equal(projectBarHeadIndex(70, 1, 'right'), 35);
});

test('zero progress starts on the strip edges', () => {
  assert.equal(projectBarHeadIndex(70, 0, 'left'), 0);
  assert.equal(projectBarHeadIndex(70, 0, 'right'), 69);
});

test('progress is clipped before projection', () => {
  assert.equal(projectBarHeadIndex(70, -1, 'left'), 0);
  assert.equal(projectBarHeadIndex(70, 2, 'right'), 35);
});
